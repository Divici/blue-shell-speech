...
- [x] **1.19 Rate limiting on login and dictation upload — PULLED FORWARD FROM 4.3.**
...
- [x] **1.22 Synthetic demo-data seeder** — **out of band, ahead of 1.20 and 1.21: sign-in
      works and every screen behind it is empty, so there is nothing to show.** A caseload
      of eight invented children, written through the domain aggregates — never raw INSERT
      — so the AAC CHECK, the filtered unique index on current notes, the immutability
      triggers and `ProviderId` on every row all hold on the way in.
      - `infra/demoseed/`, a standalone console tool in `infra/dbgrant`'s shape. **It is not
        a code path that is switched off; it is not a code path.** The api image is built
        from a Docker context of `./api` and publishes `Practice.Api`'s transitive closure,
        and this project is outside both — verified by grepping the built image. A hosted
        service behind an environment variable would have been one blade-field away from
        writing fictional children into production (D099).
      - Second guard, at run time: it **refuses a database holding any patient or enquiry
        the roster does not name**, before the first insert, and reports counts rather than
        names. Third: the database name is a required argument and must match the connection
        string's `Initial Catalog`, so a stale `BLUESHELL_MIGRATIONS_CONNECTION` is a
        refusal instead of a write.
      - Idempotent per phase. A second run creates nothing; the calendar re-anchors to the
        practice-local date of the run, so `/today` is a full day whenever it is seeded.
      - It writes **no `AuditEvent` rows**. Demo history is invented, and an invented audit
        trail is the one kind of synthetic data that would mislead the person reading it.

- [x] **1.23 Pending and loading states across the authenticated app** — **out of band,
      reported from a live sign-in: nothing on any screen said it was working.** Every one
      of these screens was built against a local database answering in single-digit
      milliseconds, so nothing ever pended in development. Deployed, the backend scales to
      zero over a database that auto-pauses and the same waits are tens of seconds.
      - **Route level.** A `loading.tsx` beside every dynamically-rendered page — nine
        behind the login plus the three sign-in steps. The set is DERIVED rather than
        listed: `app/loading-coverage.test.ts` walks `app/` and requires a fallback wherever
        the page or a layout above it declares `force-dynamic`, so a new screen inside
        `app/(app)` arrives inside the guard instead of being remembered into it.
      - **Action level.** `Sign note` and `Discard this empty note` were bare `<button>`s
        with no pending state and no disabled attribute — on the two irreversible writes in
        the product. Sign out had neither either. Fixed, and the note editor moved off
        `useFormStatus` onto per-action flags, because one form with two actions relabelled
        `Save draft` as "Saving…" throughout a signature.
      - **Transition level.** The enquiry status tabs and the day arrows are `<Link>`s on
        purpose and stay links; `useLinkStatus` marks the one that was pressed, because the
        selected tab is derived from the search parameter the render is still waiting on.
      - **The sign-in form was not the defect.** Measured against a held route in three
        engines: it disables and relabels correctly, and always did. What was missing was
        everything after it — `signIn` redirects, and none of its destinations had a
        fallback. D100.
...
- [ ] **4.3 Rate limiting on login and dictation upload** — **the login half and the
      dictation-upload policy shipped as 1.19** (D098): counters in `RateLimitCounters` in
      Azure SQL, partitioned by forwarded source AND by the address that was submitted, a
      contentless 429 with no `Retry-After`, audited once per partition per window, and a
      route-table guard that fails on the first route under `/dictation` mapped without a
      limit.
      **What remains here is one thing:** the public consultation form is still limited in
      `web` only, in process memory (`web/lib/rate-limit.ts`, whose own docstring states the
      multi-replica limit). Move it onto the shared store — the derivation it needs is
      already extracted to `web/lib/client-key.ts`, so this is a `web` → `api` call and a
      policy, not a design. Do it after 2.5, so the upload endpoint lands on the limiter
      before a second caller starts changing it.
...
- 2026-08-25 · 1.19 rate limiting on login and dictation upload · **the counter is a table, because an in-process one is the control 1.18 refused to ship.** `api` scales horizontally and to zero, so a dictionary — or `AddRateLimiter`'s built-in fixed window, which caches a limiter instance per partition inside the middleware — limits ONE replica and forgets everything when that replica is scaled away. Redis lost on three grounds at once (a new billed resource on a free-offer project, a new HIPAA-eligibility question on top of the open Container Apps one, and a second thing that has to be reachable for a login to work); blob storage lost for having no atomic counter; ingress/WAF lost for being unable to see the account dimension at all. `RateLimitCounters` in the existing Azure SQL, **one statement the engine serialises** — `UPDATE … SET [Count] = CASE WHEN [WindowEndsAtUtc] <= @now THEN 1 WHEN [Count] > @limit THEN [Count] ELSE [Count] + 1 END`, insert-if-absent under `(UPDLOCK, HOLDLOCK)` in the same batch, `MERGE` rejected for its concurrency history. Measured: **40 simultaneous requests against a limit of 5 produce 35 refusals**; falsified to a read-modify-write whose read is its own round trip, 32 — three increments lost. **Two dimensions, and the account one is the address that was TYPED, hashed, not an account that was found** — that single choice is the task, because it is what gives the unknown-address branch a bucket at all and what gives it the same bucket SHAPE as a real one, which is what stops the new control becoming the enumeration oracle 1.18 F1 measured in three dimensions. Source 20/5min, account 10/15min — looser than the five-failure lockout on purpose, so the lockout stays the binding constraint for a real account. **The source survives the BFF hop by being derived one tier out**: the browser never reaches `api`, so a socket-level key would limit the BFF and throttle Michelle alone; `clientKey()` was EXTRACTED from `app/consultation/actions.ts` into `web/lib/client-key.ts` rather than written a second time, so one `hashClientId(clientIdentifier(x-forwarded-for))` now keys the consultation limiter, fills `ConsultationRequest.SourceIpHash` and partitions this (D080's one-derivation argument, across the hop). `api` accepts it only as exactly 64 lowercase hex — **21 requests with 21 made-up keys are refused at the 21st; with the validation deleted all 21 pass**, which is D080's finding reachable through a header. `web` → `api` caller identity is still 4.4 and network isolation alone, so anything inside the environment can pick its own source bucket — it cannot pick its account bucket, which is the second reason both dimensions exist. **The 429 reveals nothing**: same status, same empty body, same headers for both dimensions and both branches, and **no `Retry-After` on either login policy** — a decision, not an omission, since there is no legitimate client that needs the number and the header would be one more field to prove identical for a known and an unknown address. `dictation-upload` DOES send it: that caller is authenticated and is a resumable uploader, so it leaks nothing and withholding it turns a client that would have waited into one that retries blind. Measured against a users table stalled 1.5s/statement: both refusals byte-identical and both answering well inside one stall, which proves at the same time that **a refused request never reaches `AspNetUsers`** — no PBKDF2 hash, no `LoginFailed` row, no `AccessFailedCount` movement. **Refusals are audited as `RateLimited` (90), once per partition per window rather than once per request** — a row per request would make `AuditEvents` the amplification target the limiter exists to close, in the one table this application never deletes from; the cost, stated, is that a burst fitting inside one window and losing its single row leaves nothing. The row carries the hashed source and **no address, not even the one that tripped the account dimension**, because a table of what somebody guessed is the enumeration list this control denies. D097's ordering rule holds and is forced here: the refusal is not a fact until the statement returns, so the row cannot precede it — and the seam takes **no `CancellationToken`** (3 of 3 attempts still counted with the connection aborted before the endpoint; falsified onto `RequestAborted`, zero). **2.5's policy is declared with its numbers and enforced by a route-table walk**, so the first route under `/dictation` mapped without a limit arrives red: requests not bytes, 256 KB chunk floor, retries spend budget, partition by PROVIDER never by a session id the caller mints, and the handler must take the request stream rather than an `IFormFile` so a refused chunk costs no bandwidth. **18 control deletions run across the 17 tests written or changed; two came back green and both were fixed** — a read-modify-write inside the same batch is serialised anyway by READ COMMITTED blocking on the previous UPDATE's lock (so the falsification had to make the read a separate round trip), and the audit-row leak test could not reach its own case because a source-partition burst never carries a submitted address, fixed by giving it a second burst on the account dimension. Also recorded: dropping the store onto the request's token fails the BUILD first (`CS9113: Parameter 'deadline' is unread`), and deleting the group's source metadata names only `/auth/mfa/enrol/begin` because the other four routes' account metadata covers for it. **What remains of 4.3** is the consultation form, still limited in `web` only and left open there rather than ticked away. API 403 → 415, vitest 469 → 481, Playwright 149 passed / 4 skipped.
- 2026-08-25 · 1.22 synthetic demo-data seeder · **the product signed in to an empty product, so this exists to make it demonstrable.** Eight invented children with guardians, addresses, goals, a week of visits and notes in every state the UI renders, plus four enquiries — all written through the domain aggregates, so the AAC CHECK, `UX_ClinicalNotes_OneCurrentPerAppointment`, the immutability triggers and `ProviderId` on every row are satisfied on the way in rather than worked around. **The production-safety question was answered structurally, not with a flag.** `ProviderSeeder`'s shape — an `IHostedService` reading configuration — is right for the one account it creates and wrong here: a seeder gated on an environment variable is a production code path that happens to be off, one blade field away from on. `infra/demoseed/` is not in the deployed image at all: the api Docker context is `./api`, the Dockerfile names its four projects, and `dotnet publish` walks `Practice.Api`'s closure, which does not contain this. Verified by grepping the built image rather than asserted. Two run-time guards behind it, because the structural one cannot see a human with the source tree and the wrong string in their shell: it **refuses a database holding any patient or enquiry the roster does not name** (counts in the message, never names — the refusal exists to protect exactly what printing them would leak), and the target database must be typed as an argument and match the connection string's own `Initial Catalog`. Provider resolution reuses D078's rule verbatim: the sole active provider, refusing when the answer is ambiguous. **It writes no `AuditEvent` rows**, and that is the decision rather than an omission — an audit row asserts a person did a thing at a time, and the table most likely to reach a SIEM is the last place to invent one. Idempotent per phase; the calendar re-anchors to the practice-local date of the run so `/today` is a full day whenever it is seeded, and a re-run the same day creates nothing. Ten tests, ten control deletions, all red: one of them contradicted its own predicted failure message — the guess was the filtered unique index, the reality was the roster's surname dictionary three phases earlier — and one stayed green until BOTH roster rows carrying `HasLegalAuthority: false` were flipped, D096's "second control covering for the first" caught in the fixture rather than in the code. D099.
- 2026-08-25 · 1.23 pending and loading states across the authenticated app · **the reported bug was the sign-in form and the sign-in form was not the defect.** Held `/auth/password` open against a real server in all three engines: the button disables, relabels to "Signing in…" and comes back — and always did. What was dead was everything AFTER it, because `signIn` ends in a redirect and none of `/login/verify`, `/login/enrol` or `/dashboard` had a route-level fallback, so the wait a person experiences as "I signed in and nothing happened" was the navigation, not the submission. **Twelve `loading.tsx` segments, and the set is derived rather than listed**: `app/loading-coverage.test.ts` walks `app/` and demands a fallback wherever the page or a layout above it declares `force-dynamic` — which is a fact about the tree, so a tenth screen inside `app/(app)` inherits that layout's directive and arrives inside the guard. The two statically prerendered pages are asserted as exempt in their own test, so the predicate is pinned from both sides; deleting the directive from `app/(app)/layout.tsx` turns nine pages "static" and goes red twice. **Two irreversible writes had no feedback at all**: `Sign note` and `Discard this empty note` were bare `<button>`s beside a `Save draft` that had both a pending label and a disabled attribute — on a screen where a signature is immutable and a discard has no undo. The editor moved off `useFormStatus` onto `useActionState`'s per-action flags, because one form with two actions relabelled `Save draft` as "Saving…" throughout a signature, which names the wrong operation on the one screen where the wrong operation cannot be taken back. **The double-submit tests were green by construction and the deletion is what found it**: React SERIALISES `useActionState` submissions, so three clicks produce one call at the moment of the assertion and three the instant the first resolves — deferred, not dropped. Every one of them now releases the held action and waits for the control to return before counting. A second finding fell out of the same investigation and cost an hour: an action left hanging when a test ends is not confined to that test — the next one's release has no effect and it fails while passing in isolation. Skeletons where the shape is known and a spinner where it is not, one vocabulary across sixteen controls; `aria-busy` and one `role="status"` per screen with the bars `aria-hidden`, because forty rectangles read out individually is worse than silence; `motion-reduce:animate-none` at the element as well as the global stylesheet. The status tabs and the day arrows stay `<Link>`s — bookmarkable, back-button friendly, no JavaScript — and `useLinkStatus` marks the one that was pressed, because the selected tab is derived from the search parameter the render is still waiting on. **The E2E harness gained an authenticated fixture**, minting the session cookie `web` itself issues under a key resolved once and shared with the server; the stub holds a reserved date and a reserved address, keyed on request content because three browser projects share one instance. Fourteen control deletions, all red. D100.
---
